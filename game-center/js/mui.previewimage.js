(function($, window) {
	...
	proto.initEvent = function() {
		...
		$(document.body).on('tap', 'img[data-preview-src]', function() {
			self.open(this);
            document.getElementById('previewImage-dot').innerHTML=null;
			...
			if(arr&&arr.length>1){
                ...
                for(i=0;i<arr.length;i++){
                    var div=document.createElement('div');
                    i==0?div.className="mui-indicator mui-active":div.className="mui-indicator";
                    frag.appendChild(div);
                }
                document.getElementById('previewImage-dot').innerHTML=null;
                document.getElementById('previewImage-dot').appendChild(frag);
                frag=null;
			}
			return false;
		});
